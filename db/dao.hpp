...
public:
    ...
    std::vector<ROW_TYPE>
    get(const std::vector<std::string> &fields, const std::map<std::string, std::string> &filters) {
        mysqlpp::Query query = conn.query();
        ...
        for (auto it = filters.begin(); it != filters.end(); ++it) {
            if (it != filters.begin())
                query << " and ";
            query << it->first << " = " << mysqlpp::quote << it->second;
        }
    ...
    std::vector<ROW_TYPE> get(const std::map<std::string, std::string> &filters) {
